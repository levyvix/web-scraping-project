...
def test_network_intermittency_simulation():
    ...
    with patch("main.Fetcher.get") as mock_get:
        ...
        book_data_list = [
            {
                "title": f"Book {i}",
                "detail_url": f"https://books.toscrape.com/book_{i}.html",
            }
            for i in range(4)
        ]
...
def test_process_book_listing_extremely_long_title():
    ...
    mock_url_element = MagicMock()
    mock_url_element.attrib = {"href": "test-book_1/index.html", "title": long_title}
...
def test_process_book_details_encoding_issues_in_response():
    ...
    with patch("main.Fetcher.get") as mock_get:
        ...
        mock_description.text.strip.side_effect = UnicodeDecodeError(
            "utf-8", b"", 0, 1, "invalid"
        )
