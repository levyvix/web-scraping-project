...
class TestMainFunctionSuccessScenarios:
    ...
    def test_single_page_scraping_workflow(
        ...
        with (
            patch("main.process_book_listing") as mock_listing,
        ...
        mock_fetcher_get.assert_called_once_with(
            "https://books.toscrape.com/", stealthy_headers=True
        )
    ...
    def test_progress_tracking_and_logging(
        ...
        assert any(
            "Extracting listings from page 1" in str(call) for call in tqdm_calls
        )
        assert any(
            "Fetching details for page 1 books" in str(call) for call in tqdm_calls
        )
...
class TestMainFunctionErrorScenarios:
    ...
    def test_initial_page_fetch_failure(
        self, mock_fetcher_get, mock_save_to_json, mock_logger
    ):
        ...
        mock_logger.error.assert_called_with(
            "Failed to fetch first page. Status code: 404"
        )
...
class TestMainFunctionEdgeCases:
    ...
    def test_books_with_empty_processing_results(
        self, mock_fetcher_get, mock_save_to_json, mock_logger, mock_tqdm
    ):
        ...
        with patch("main.process_book_listing") as mock_process_listing:
            ...
            with patch("main.process_book_details") as mock_process_details:
                ...
                with patch(
                    ...
                    listing_future1.result.return_value = {
                        "title": "Valid Book 1",
                        "price": "£10.00",
                    }
                    ...
                    listing_future3.result.return_value = {
                        "title": "Valid Book 2",
                        "price": "£15.00",
                    }
                    ...
                    detail_future1.result.return_value = {
                        "title": "Valid Book 1",
                        "price": "£10.00",
                        "description": "Desc 1",
                    }
                    detail_future2 = MagicMock()
                    detail_future2.result.return_value = {
                        "title": "Valid Book 2",
                        "price": "£15.00",
                        "description": "Desc 2",
                    }
                    ...
                    mock_context.submit.side_effect = [
                        listing_future1,
                        listing_future2,
                        listing_future3,  # Listing futures
                        detail_future1,
                        detail_future2,  # Detail futures (only for valid books)
                    ]
                    ...
                    with patch(
                        ...
                        mock_as_completed.side_effect = [
                            ...
                            [
                                detail_future1,
                                detail_future2,
                            ],  # Detail processing (only valid books)
                        ]
        ...
        assert all(
            book.get("title") for book in saved_data
        )  # All saved books have titles
    ...
    def test_max_pages_parameter_limiting(
        ...
        actual_get_calls = [
            call
            for call in mock_fetcher_get.call_args_list
